...
<body>
	<header
		style="display: flex; justify-content: space-between; background-color: rgb(60, 196, 254);">
		<h1 style="flex-grow: 1; padding: 1%;">SSAFIT</h1>
		<nav style="padding: 1%;">

			<a href="main?act=index"><i class="bi bi-house-door-fill fs-1"></i></a>
			<a href="main?act=reviewList"><i class="bi bi-search fs-1"></i></a> <a
				href="main?act=registerForm"><i class="bi bi-person-circle fs-1"></i></a>
			<a href="main?act=loginForm"><i
				class="bi bi-box-arrow-in-right fs-1"></i></a>
					...
					<table id="video-list">
						...
						<c:forEach items="${sessionScope.list}" var="video">
							<tr>
								<td></td>
								<td>${video.viewCnt}</td>
								<td><a href="main?act=detail&id=${video.id}">${video.title}</a>
								</td>
								<td>${video.part}</td>
					...
					<div class="collapse multi-collapse" id="collapseExample1">
						<div class="card card-body">
							...
							<table id="video-list">
								...
								<c:forEach items="${fullyBody}" var="video">
									<tr>
										<td></td>
										<td>${video.viewCnt}</td>
										<td><a href="main?act=detail&id=${video.id}">${video.title}</a>
					...
					<div class="collapse multi-collapse" id="collapseExample2">
						<div class="card card-body">
							...
							<table id="video-list">
								...
								<c:forEach items="${upperBody}" var="video">
									<tr>
										<td></td>
										<td>${video.viewCnt}</td>
										<td><a href="main?act=detail&id=${video.id}">${video.title}</a>
					...
					<div class="collapse multi-collapse" id="collapseExample3">
						<div class="card card-body">
							...
							<table id="video-list">
								...
								<c:forEach items="${lowerBody}" var="video">
									<tr>
										<td></td>
										<td>${video.viewCnt}</td>
										<td><a href="main?act=detail&id=${video.id}">${video.title}</a>
